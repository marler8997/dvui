...
comptime { std.debug.assert(dvui.backend_kind == .win32); }
const win32 = struct {
...
pub fn main() !void {
    ...
    //const backend = dvui.backend.create();

    const window_class = win32.L("Win32Standalone");
    try dvui.backend.registerClass(window_class, .{});

    const backend_window = try dvui.backend.createWindow(.{
        .size = .{ .width = 800.0, .height = 600.0 },
        .title = win32.L("DVUI Win32 Standalone Example"),
        .class = window_class,
    });
    _ = backend_window;

    // init dvui Window (maps onto a single OS window)
    var win = try dvui.Window.init(@src(), gpa, backend_window, .{});
    defer win.deinit();

    dvui.backend.runMainLoop();
